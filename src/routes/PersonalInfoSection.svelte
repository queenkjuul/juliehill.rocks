...
>
  ...
  <div class="flex w-full flex-col items-center justify-center gap-2">
    <Heading tag="h1" class="text-center text-primary-400 dark:text-primary-400 md:text-left"
      >{firstName} {lastName}</Heading
    >
    <Heading tag="h2" class="text-center md:text-left">{occupation}</Heading>
    {#if location}
      <Heading tag="h6" class="text-center md:text-left">{location}</Heading>
    {/if}
    <About />
  </div>
</div>
